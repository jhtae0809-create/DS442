...
# Replace FILENAME with your homework file name (eg. homework3_abcd1234)
import homework3_hzj5293 as homework
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                square.bind("<Button-1>",
                    lambda event, row=row, col=col: self.execute_move(row, col))
                row_squares.append(square)
            self.squares.append(row_squares)

    def execute_move(self, row, col):
        if self.game.is_legal_move(row, col, self.vertical):
            self.game.execute_move(row, col, self.vertical)
...
class DominoesGUI(Tkinter.Frame):
    ...
    def __init__(self, master, rows, cols):
        ...
        self.game = homework.make_dominoes_game(rows, cols)
        self.rows = rows
        self.cols = cols
    ...
    def perform_random_move(self):
        if not self.game.game_over(self.board.vertical):
            ...
            self.board.execute_move(row, col)

    def perform_best_move(self, limit):
        if not self.game.game_over(self.board.vertical):
            (row, col), best_value, total_leaves = \
                self.game.get_best_move(limit, self.board.vertical)
            self.board.execute_move(row, col)
